    ...
    describe('when called with', () => {
        ...
        describe('class MissingTokenEx', () => {
            test('throws expected Exception', async() => {
                ...
                try {
                    await tryCatchWrap(MissingTokenEx, handler);
                } catch ($ex) {
                    expect($ex).toBeInstanceOf(Exception);
                    expect($ex).toBeInstanceOf(AuthException);
                    expect($ex).toBeInstanceOf(AuthenticationException);
                    expect($ex).not.toBeInstanceOf(AuthorizationException);
                    expect($ex.message).toBe(ExceptionMessageDictionary[EX_AUTH_MISSING_TOKEN]);
                    expect($ex.code).toBe(EX_AUTH_MISSING_TOKEN);
                    expect($ex.err).toBe($err);
                }
            });
        });
        describe('class NotHasAbilityEx', () => {
            test('throws expected Exception', async() => {
                ...
                try {
                    await tryCatchWrap(NotHasAbilityEx, handler);
                } catch ($ex) {
                    expect($ex).toBeInstanceOf(Exception);
                    expect($ex).toBeInstanceOf(AuthorizationException);
                    expect($ex).not.toBeInstanceOf(AuthenticationException);
                    expect($ex.message).toBe(ExceptionMessageDictionary[EX_AUTH_NOT_HAS_ABILITY]);
                    expect($ex.code).toBe(EX_AUTH_NOT_HAS_ABILITY);
                    expect($ex.err).toBe($err);
                }
            });
        });
        describe('class NotHasOwnershipEx', () => {
            test('throws expected Exception', async() => {
                ...
                try {
                    await tryCatchWrap(NotHasOwnershipEx, handler);
                } catch ($ex) {
                    expect($ex).toBeInstanceOf(Exception);
                    expect($ex).toBeInstanceOf(AuthorizationException);
                    expect($ex).not.toBeInstanceOf(AuthenticationException);
                    expect($ex.message).toBe(ExceptionMessageDictionary[EX_AUTH_NOT_HAS_OWNERSHIP]);
                    expect($ex.code).toBe(EX_AUTH_NOT_HAS_OWNERSHIP);
                    expect($ex.err).toBe($err);
                }
            });
        });
